...
//main game engine
function gameEngine() {
  //if snake has not started moving just draw the boared
  if (inputDir.x === 0 && inputDir.y === 0) {
    render();
    return;
  }
  //calculate new3 hwead positiojnn
  const newHead = {
    x: snake[0].x + inputDir.x,
    y: snake[0].y + inputDir.y,
  };

  //check for Collisioni
  if (willCollide(newHead)) {
    gameOver();
    return;
  }

  // move snale foeward by  adding new hwead
...
//render snake and the food on the board
function render() {
  board.innerHTML = "";
  ...
  foods.forEach((f) => {
    ...
    e.className =
      f.val === 1 ? "food-small" : f.val === 2 ? "food-mid" : "food-big";
    board.appendChild(e);
  });
...
//when the player start overlay start buttons
...
//adding pause restart and mutw control events
